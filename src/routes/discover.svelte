      ...
      {#await spotifyTracks then spotifyTracks}
         <div class=" left-0 z-[50]   w-full  overflow-hidden rounded-xl">
            <div class="flex flex-row justify-center text-xl font-light align-baseline items-center">
               ...
               <p class="whitespace-nowrap px-2">top fourty songs in the us <span class="text-xs">and if they're tabbed/chorded</span></p>
               <div class="h-[2px]  bg-black w-full"></div>
            </div>

            <div class="grid basis-1/4 grid-cols-10 gap-2">
               {#each spotifyTracks.tracks.items.slice(0, 40) as track, index}
                  <a href="/track/{track.track.id}/tabs" class="flex flex-col  items-center px-1 rounded-xl h-full">
                     <div class="relative text-center group">
                        <div class="transition ease-in-out duration-500 text-[30px] absolute group-hover:opacity-0 left-2 top-1/4">
                           <p class="">❌</p>
                        </div>
                        <div class="transition ease-in-out duration-500 text-[30px] absolute group-hover:opacity-0 right-2  top-1/4">
                           <p class="">✅</p>
                        </div>
                        <div
                        ...
                        <img class="w-24 rounded-xl " src="{track.track.album.images[0].url}" alt="" />
                     </div>
                  </a>
               {/each}
            </div>
...
{#await chordedTracks}
   <div class="absolute left-0 bottom-[115px] z-[60]  w-full overflow-hidden text-xl font-light" style="">
      <p class="whitespace-nowrap text-xs">{Array(21).fill("loading...").join("⸺")}</p>
